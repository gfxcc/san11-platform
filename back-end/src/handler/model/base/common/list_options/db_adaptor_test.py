...
class TestPostgresAdaptor(unittest.TestCase):
    ...
    def test_gen_order_by(self):
        ...
        self.assertEqual(statement1, "ORDER BY data->>'f1'")
        self.assertEqual(statement2, "ORDER BY data->>'f1' DESC, data->>'f3'")

    def test_gen_where_all_comp_ops(self):
        s1, p1 = self.adaptor.gen_where(ListOptions(None, 1, 1, '', 'f1 = 123'))
        self.assertEqual(s1, "WHERE data->>'f1' = %(f1)s")
    ...
    def test_gen_where_complex(self):
        ...
        self.assertEqual(s, "WHERE (data->>'f1' >= %(f1)s) OR (((data->'f2')::jsonb ? %(f2)s) AND (data->>'f3' = %(f3)s)) AND (data->>'f4' > %(f4)s)")
        self.assertEqual(p, {
            'f1': 123,
            'f2': 456,
            'f3': 'OK',
