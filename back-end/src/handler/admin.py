...
class AdminHandler:
    def get_admin_message(self, request, context):
        ...
        sql = 'SELECT parent, text, create_time FROM comments ORDER BY create_time desc limit 10'
        message['new-comments'] = list(map(lambda fields: ', '.join(map(str, fields)), run_sql_with_param_and_fetch_all(sql, {})))
        sql = 'SELECT comment_id, text, create_time FROM replies ORDER BY create_time desc limit 10'
