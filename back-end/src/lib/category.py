...
from .db_util import run_sql_with_param_and_fetch_one
...
class PrimaryCategory(Category):
    ...
    @classmethod
    def from_name(cls, name: str) -> Category:
        sql = 'SELECT category_id, name FROM primary_categories WHERE name=%(name)s'
        resp = run_sql_with_param_and_fetch_one(sql, {'name': name})
        return cls(resp[0], resp[1])
    
    @classmethod
    def from_category_id(cls, category_id: int) -> Category:
        sql = 'SELECT category_id, name FROM primary_categories WHERE category_id=%(category_id)s'
...
class SecondaryCategory(Category):
    @classmethod
    def from_name(cls, name: str) -> Category:
        sql = 'SELECT category_id, name FROM secondary_categories WHERE name=%(name)s'
        resp = run_sql_with_param_and_fetch_one(sql, {'name': name})
        return cls(resp[0], resp[1])

    @classmethod
    def from_category_id(cls, category_id: int) -> Category:
        sql = 'SELECT category_id, name FROM secondary_categories WHERE category_id=%(category_id)s'
        resp = run_sql_with_param_and_fetch_one(sql, {'category_id': category_id})
        return cls(resp[0], resp[1])
