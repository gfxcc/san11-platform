...
from core.common.credentials import (AwsCredentials, get_aws_credentials,
...
class FileServer(ABC):
    credentials_environment_variable: Optional[str] = None
    ...
    def _skip_unconfigured_cleanup(self, operation: str) -> bool:
        environment_variable = self.credentials_environment_variable
        if environment_variable and not os.environ.get(environment_variable):
            message = (
                f'{type(self).__name__} integration is not configured: '
...
class Gcs(FileServer):
    credentials_environment_variable = 'GCLOUD_CREDENTIALS_FILE'
    ...
    def delete_file(self, bucket_class: BucketClass, filename: str, force: bool = False) -> None:
        ...
        if self._skip_unconfigured_cleanup(f'GCS file deletion for {filename}'):
            return
        storage_client = self._get_client()
        bucket = storage_client.bucket(self.get_bucket_name(bucket_class))
    ...
    def delete_by_prefix(self, bucket_class: BucketClass, path: str, force: bool = False) -> None:
        ...
        if self._skip_unconfigured_cleanup(f'GCS prefix deletion for {path}'):
            return
        storage_client = self._get_client()
        bucket = storage_client.get_bucket(self.get_bucket_name(bucket_class))
...
class S3(FileServer):
    ...
    def __init__(self) -> None:
        ...
        if not os.environ.get(self.credentials_environment_variable):
            logger.warning(
    ...
    def delete_file(self, bucket_class: BucketClass, filename: str, force: bool = False) -> None:
        super().delete_file(bucket_class, filename, force)
        if self._skip_unconfigured_cleanup(f'S3 file deletion for {filename}'):
            return
        client = self._get_client()
        client.delete_object(
            Bucket=self.get_bucket_name(bucket_class),
    ...
    def delete_by_prefix(self, bucket_class: BucketClass, path: str, force: bool = False) -> None:
        super().delete_by_prefix(bucket_class, path, force)
        if self._skip_unconfigured_cleanup(f'S3 prefix deletion for {path}'):
            return
        bucket = self._get_resource().Bucket(self.get_bucket_name(bucket_class))
